...
const findCharacteristicUUIDs = async (device: Device) => {
  ...
  for (const service of services) {
    ...
    if (writableCharacteristicUUID && notifiableCharacteristicUUID) {
      ...
      return { serviceUUID: service.uuid, writableCharacteristicUUID, notifiableCharacteristicUUID };
    }
  }
...
export const initializeOBD = async (device: Device) => {
  try {
    ...
    for (const command of initCommands) {
      await writeCommand(device, serviceUUID, writableCharacteristicUUID, command);
